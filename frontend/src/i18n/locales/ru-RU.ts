...
/example\.(net|org)/`,
  ...
  datasource: {
    ...
    connectorDesc: {
      ...
      git: 'Синхронизация документов из Git-репозитория по HTTPS или SSH',
    },
    field: {
      appId: 'App ID',
      ...
      gitLatestParent: 'Родитель каталога версий (необязательно)',
      gitLatestPattern: 'Шаблон каталога версии (необязательно)',
      gitLatestPairHint: 'Заполните оба поля для выбора новейшего каталога, например versions + version-*.',
      gitExtensions: 'Расширения файлов',
      gitExtensionsHint: 'Список расширений для синхронизации через запятую.',
      gitUsername: 'Имя пользователя HTTPS (необязательно)',
      gitToken: 'Токен доступа HTTPS (необязательно)',
      gitTokenHint: 'Требуется только для приватных HTTPS-репозиториев.',
      gitPrivateKey: 'Закрытый ключ SSH (необязательно)',
      gitKnownHosts: 'SSH known_hosts (обязательно для SSH)',
      gitKnownHostsHint: 'Вставьте проверенную строку ключа хоста. Проверка ключа SSH не отключается.',
    },
    comingSoon: 'Скоро',
