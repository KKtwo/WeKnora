...
WeKnora 的数据源导入模块支持从飞书、钉钉、Notion、语雀、RSS 和通用 Git 仓库自动导入和同步内容到知识库。用户可配置数据源连接，选择需要同步的资源，并通过手动触发或定时调度自动完成内容的增量/全量同步。

飞书支持企业应用 tenant token 与个人 OAuth 两种身份；个人 token 在过期前自动刷新并加密回写。钉钉采用个人 OAuth，并通过钉钉官方文档、表格、AI 表格和钉盘 MCP 网关浏览与读取内容。钉钉增量游标保存内容 hash，来源资源消失时产生删除事件。
...
通用 Git Connector 适用于 GitHub、GitLab、Gitea 和公司内部 Git 服务，不依赖某一家平台 API。

1. 进入知识库设置页 → **数据源** → **添加数据源** → **Git 仓库**。
2. 填写仓库地址和分支。支持 `https://`、`ssh://`、`user@host:path`，不支持本地路径、`file://` 和明文 HTTP。
3. 私有 HTTPS 仓库填写用户名和访问令牌；SSH 仓库可填写私钥，并且必须提供已核验的 `known_hosts` 主机公钥。
4. 可通过 `path` 限定默认目录，或同时配置 `latest_parent` 与 `latest_pattern`，自动选择自然版本号最新的目录。
5. 测试连接后选择要同步的文件或目录，并启用增量同步和同步删除。

公司内网 Git 域名默认会被 SSRF 防护拒绝，部署管理员需要把准确域名加入 `SSRF_WHITELIST_EXTRA`；不要使用大网段白名单。

Markdown 中的相对图片会随文档一起入库：先按 Markdown 文件所在目录解析，找不到时再按仓库根目录解析。
支持 JPEG、PNG、GIF、WebP 和 BMP；单图最多 10 MB、单篇最多处理 30 张，且内嵌后的文档仍受数据源文件大小上限约束。
列表项正文下一行使用一个 Tab 或四个空格缩进的图片仍会正常入库；围栏代码、行内代码、HTML 注释和真正的缩进代码块中的图片示例不会触发图片读取。
HTTP(S) 和已有存储协议图片继续由原解析链路处理。

增量游标保存当前 commit、文件 blob ID 和 Markdown 的图片 blob 依赖。后续同步仅读取新增或变更文件；图片新增、
替换或删除也会自动重建引用它的 Markdown，并为消失的文档路径发送删除事件。知识条目使用
`tenant_id + knowledge_base_id + datasource_id + external_id` 复合身份匹配，因此不同数据源中相同路径不会互相覆盖或误删。
...
| 连接器类型 | 凭证字段 | 认证方式 |
|-----------|---------|---------|
| 飞书 (feishu) | `auth_mode`, `app_id`, `app_secret`，个人模式另含 OAuth token | Tenant Token / User OAuth2 |
| 钉钉 (dingtalk) | `client_id`, `client_secret`，OAuth token | User OAuth2 |
